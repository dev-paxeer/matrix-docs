---
title: "EVM Client"
description: "The EVM client wraps go-ethereum RPC operations for chain interaction: tx building, gas estimation, signing, raw broadcast, receipt polling, and debug tracing."
---
...
type Client struct {
    rpcURL  string
    chainID *big.Int // cached from first successful lookup
}
```
...
1. Fetches latest header; if `BaseFee` is present, uses dynamic fee tx (`types.DynamicFeeTx`)
2. `GasTipCap` = suggested tip (fallback to 1 gwei)
3. `GasFeeCap` = `2*BaseFee + tip` (standard headroom for one base-fee bump)
4. If header lacks `BaseFee`, falls back to legacy tx (`types.LegacyTx`) with `GasPrice`
...
`WaitReceipt` polls every 2 seconds until the receipt is found or context is cancelled. This is a simple blocking wait suitable for daemon use.

### Debug tracing

`TraceCall` uses the raw `rpc.Client` (not `ethclient`) to call `debug_traceCall` with `disableStorage: true`. This reduces trace size and is sufficient for most agent debugging needs. Returns raw JSON (`any`) so the caller handles formatting.
...
`SignTxKey` uses `types.LatestSignerForChainID` to select the correct signer (EIP-155, EIP-2930, EIP-1559) and returns the RLP-encoded raw tx + the signer address.

### Revert reason helper

```go
func EncodeRevertReason(data []byte) string
```

Returns `0x`-prefixed hex of revert data for agent debugging.
...
|---|---|
| Add connection pooling | `internal/evm/client.go` - add persistent client |
| Change gas fallback | `internal/evm/evm.go` - `BuildTx` gas default (3M) |
| Add batch RPC | `internal/evm/client.go` - new method using `rpc.BatchCall` |
| Change receipt poll interval | `internal/evm/client.go` - `WaitReceipt` ticker (2s) |
