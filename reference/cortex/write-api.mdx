---
title: "Write API"
description: "Package matrix/cortex exposes four mutating primitives: Write, Update, Tombstone, and UpdateHead. Every call commits an atomic Pebble batch."
---
...
**Atomicity is non-negotiable.** Every write uses a single `store.BeginWrite` to `AppendJournal` to `Commit` batch. There is no path that mutates a store key without a corresponding journal entry; `Commit` enforces this via `ErrBatchNoJournal`.
...
uri, err := c.Write(
    memory.Head{
        ...
        Frames: []memory.FrameRef{
            {Verb: memory.VerbAnalyze, ObjKind: memory.ObjService, ObjRef: "paxeer-net"},
        },
    },
    memory.FactData{
...
idx/frame/<…>/<id>      <- one entry per FrameRef on the Head (all types)
idx/actor_obj/<…>/<id>  <- one entry per FrameRef if Type == Event
...
The `MemoryHead` is rewritten with `Tombstoned` set. Version records are NOT deleted, they remain readable for audit via `Resolve(uri)`.
...
| `memory.ErrTypeDataMismatch` | `Update` data type does not match URI type |
| `memory.ErrTombstoned` | `Update` or `UpdateHead` on a tombstoned memory |
| `memory.ErrNotFound` | `Resolve` cannot find the head or version |
