---
title: Store & Journal
description: "The per-actor Pebble database, atomic write-batch machinery, key encoding, and the append-only journal that anchors cortex replay."
---
...
**Multiple journal entries per batch.** `WriteBatch` supports multiple `AppendJournal` calls within a single `BeginWrite`. Required because `cortex.Attest` emits `KindAttest` at seq=N and `KindLearnWeights` at seq=N+1 atomically. Internally each `AppendJournal` call allocates its own seq, stages its own MMR leaf via the `JournalHook`, and uses an indexed Pebble batch (`BatchedReader`) so in-batch staged nodes are visible to subsequent leaf cascades.
...
// Journal iteration (walks from seq 0 to head)
...
The hook fires per `AppendJournal` call, inside the same `WriteBatch`. It receives the raw Pebble batch, the allocated seq, and the leaf hash so it can stage additional keys without a separate commit.
...
wb.AppendJournal(entry)     // at least once; may be called multiple times (Attest)
...
- All numeric components are big-endian fixed-width so byte-sort equals numeric-sort.
...
| `meta/snapshot_seq` | Next snapshot seq to allocate (uint64 BE) |
...
**Sidecar posture:** `meta/salience_weights`, `meta/compile_cache/*`, `meta/goal_state/*`, `meta/embed_cursor`, `meta/embed_vertex_next`, and `meta/embed_model` are derived / runtime-policy state. They are NOT inputs to `OverallRoot`. The `Rebuild` operation drops and re-derives them from the journal.
...
| `find_late` | `cortex.Find` with `LateBinding=true` - D13 audit hook. Payload includes `AccessedIDs[]` for salience replay. |
| `scope_violation` | Sub-agent read/write outside its `CortexScope` |
| `attest` | `cortex.Attest` - intent outcome + cited memory IDs (post-tombstone-skip) |
| `learn_weights` | Always immediately follows `attest` in the same atomic batch - EMA weight delta |
| `gc` | Future version GC sweep |
| `migration` | Future schema migration step |

### KindAttest and KindLearnWeights pairing

`cortex.Attest` emits two journal entries in a single atomic batch: `KindAttest` at seq=N and `KindLearnWeights` at seq=N+1. The seq-pairing is unconditional: even when the EMA step degenerates (empty cited set, all-zero factor profile), `KindLearnWeights` is still emitted with `Skipped=true` and `NewW* == PrevW*`. The `BatchedReader` interface on the indexed Pebble batch allows the second MMR leaf to read the first leaf's staged nodes.
...
Walks `j/` in seq order from 0 to head. Used by the embedding worker, replay rebuild, and audit tooling. The gap-free invariant means every seq from 0 to `s.NextSeq()-1` has a corresponding entry.
...
| Support multi-entry atomic batches | `store/writebatch.go` - `WriteBatch` already supports multiple `AppendJournal` calls per `BeginWrite` |
