---
title: "Cortex: Developer Reference"
description: "Cortex is Matrix's per-actor persistent memory store, a Pebble-backed KV database with a typed taxonomy, salience-ranked queries, async embeddings, Merkle snapshots, and a full replay invariant."
---
...
|---|---|
| [Memory Taxonomy](/reference/cortex/memory-taxonomy) | The 9 typed memory kinds, Head, Version, Forms, Tombstone |
| [Store & Journal](/reference/cortex/store-and-journal) | Per-actor Pebble DB, key encoding, atomic write batches, journal |
| [Write API](/reference/cortex/write-api) | `Write`, `Update`, `Tombstone`, `UpdateHead` mutations and their journal footprint |
| [Find Query](/reference/cortex/find-query) | The typed query engine, predicates, planner, ordering, graph traversal |
| [Context Bundle](/reference/cortex/context-bundle) | The three-tier cold-start composer, Pinned, Frame-relevant, Outcomes |
| [Embedder & Vector](/reference/cortex/embedder-and-vector) | Async embedding worker, pure-Go HNSW, `Find Near` / `Find NearURI` |
| [Edges & Graph](/reference/cortex/edges-and-graph) | Typed edges, `AddEdge`, `RemoveEdge`, bounded BFS |
| [Snapshot & Proofs](/reference/cortex/snapshot-and-proofs) | Journal MMR, per-namespace SMTs, `OverallRoot`, multi-proofs |
| [Salience](/reference/cortex/salience) | The 5-factor formula, cold weights, EMA per-actor weight learning |
| [Attest & Compact](/reference/cortex/attest-and-compact) | Intent attestation, salience feedback, budget-aware compaction |
| [Scope](/reference/cortex/scope) | Sub-agent `CortexScope`, Merkle-authenticated read/write gating |
| [Replay](/reference/cortex/replay) | Drop-and-rebuild, the derivability invariant and `Cortex.Rebuild` |
...
├── rebuild.go              # Cortex.Rebuild facade, delegates to replay.Rebuild
...
│   └── writebatch.go       # BeginWrite, AppendJournal (multi-entry), Commit / Abort
├── salience/
...
│   ├── rebuild.go          # rebuildMemoriesIndexes, rebuildEdgesSMT, rebuildJournalMMR, rebuildSalienceFromJournal
...
These decisions are frozen. Do not re-litigate them without a documented protocol-level change.

| ID | Decision |
|---|---|
| **D4** | Cortex holds no key material. Signing lives in the agent runtime or tools layer. |
| **D11** | Compiler seed = sha256(intent.id \|\| actor \|\| OverallRoot \|\| mtx_digest \|\| model_digest). Same inputs produce same output. |
