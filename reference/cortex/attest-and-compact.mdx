---
title: "Attest & Compact"
description: "cortex.Attest records intent outcomes and feeds salience feedback via EMA weight learning. cortex.Compact runs budget-aware context compaction."
---

`cortex.Attest` records the outcome of a completed intent and feeds that outcome back into the salience of every memory the plan cited, including EMA weight learning. `cortex.Compact` runs budget-aware context compaction, summarizing non-load-bearing memories into short stubs and emitting a journal checkpoint for re-entry.
...
1. Pre-resolves every cited URI to a live (non-tombstoned) Head. Tombstoned/missing URIs are silently skipped (land in `SkippedURIs`).
...
type AttestOpts struct {
    IntentID  string            // required; empty returns ErrAttestEmptyIntentID
    Outcome   AttestOutcome     // AttestOutcomeSuccess or AttestOutcomeFailure
    Reason    string            // free-form; only factual_error/wrong_assumption trigger -1
    Cited     []memory.URI      // up to MaxCitedURIsPerAttest (256)
    CreatedBy string            // agent ref for journal audit
}
```
...
type AttestResult struct {
    ...
    PrevWeights    salience.Weights  // weights before this attest
    NewWeights     salience.Weights  // weights after EMA step
...
- **`KindAttest`** at seq=N: contains the post-skip affected IDs (`CitedIDs[]`), outcome, and reason. The replay harness uses this to re-apply salience bumps deterministically.
- **`KindLearnWeights`** at seq=N+1: contains the before/after weight vector (`NewWR/WA/WC/WD/WV`), the EMA alpha, and whether the step was skipped. The replay harness uses this to re-apply the EMA step.
...
`Attest` is protected by a per-(actor, intent_id) token bucket (1/sec, burst 5) to bound the Pebble-sync + dual-journal-entry cost from a malicious sub-agent. Over-rate calls return `ErrAttestRateLimited` without any side effects (no journal entries, no salience bumps, no EMA step).
...
Cortex auto-protects pinned items (Identity, hard Constraints, active Goals) present in `InContext`, in addition to the caller's explicit `LoadBearing` URI list. The full Kept set = caller-listed URIs union cortex-auto-pinned URIs.
...
type CompactedItem struct {
    ...
    Salience  float32   // from salience cache at compaction time; NOT bumped
}
```
...
```go
uri := cortex.BuildCheckpointURI(intentID, stepID)
// -> "matrix://journal/logs/<intent_id>/<step_id>"
```

Kind = "logs", matching research/01 §4.10 workspace taxonomy.
...
p50 < 100 ms, ceiling < 400 ms. All reads are point reads on `m/`, `mv/`, `salience/`; no vector / no traversal / no idx scan.
