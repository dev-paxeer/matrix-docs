---
title: "Salience"
description: "Package matrix/cortex/salience computes and persists the per-memory ranking signal. Five-factor formula, cold weights, EMA per-actor weight learning."
---
...
**Salience is NOT in OverallRoot.** The `salience/<id>` and `meta/salience_weights` keys are derived state. The `Rebuild` operation drops and re-derives them from the journal. This is intentional, the recency component `R(m)` depends on wall-clock time, so the same salience cache could not be reproduced byte-identically on replay anyway. Different rebuild clocks produce different caches and identical `OverallRoot`s, by design.

**Learned weights override cold weights at query time.** `Find`, `Context`, and `Compact` load the per-actor `Weights` via `ReadWeights` once and use `ColdScoreWith` for all ranking. Cold-start actors (no `meta/salience_weights` key) use the §8.2 defaults.
...
R(m) = exp(-dt(now, last_used) / half_life)           // recency; half-life 90d
...
`HalfLifeNanos` = 90 days. Despite the label, this is a 1/e decay (R drops to approximately 0.368 at one constant, not 0.5). Adopting the spec formula verbatim keeps `cortex_snapshot_hash` byte-identical with any independent re-implementation.

### Access saturation

`AccessSaturation = 1000.0`. Fixed constant (not per-actor max) because the spec's max-based formula is unstable under cold start and creates ranking discontinuities when a single memory pulls the max upward.
...
type Score struct {
    ...
    AccessCount   uint64   // incremented by Find (LateBinding) and Attest (success)
    Citations     uint64   // citation count in successful plans
    Importance    uint8    // DeclaredImportance at last bump; 0..10
    Pinned        bool     // true for Pinned-tier memories
    Cached        float32  // last computed cold score (snapshot; not live ranking)
    ComputedAt    int64    // UnixNano of last Cached recompute
}
```

All factor inputs are stored alongside the cached score so a recompute triggered by a single input change (e.g. attestation citation update) is O(1) without needing to re-scan the head/version.
...
| `BumpForAccess(sc, now)` | `Find` with `LateBinding=true` | Increments `AccessCount`, refreshes `Cached` |
...
```go
const EMARate float32 = 0.05  // alpha
...
`alpha=0.05` is intentionally slow, roughly 14 intents to move the weight by half a standard deviation from its initial value. `EMARate` is stamped on every `KindLearnWeights` journal entry for audit-replay determinism.

### Cold start

`ReadWeights` returns `(DefaultWeights(), false, nil)` when `meta/salience_weights` is absent. The `false` return indicates no learned weights exist yet.
...
err = salience.DecodeWeights(bytes, &weights)
```
...
| Half-life decay constant | `cortex/salience/salience.go` - `HalfLifeNanos` |
...
| Access saturation constant | `cortex/salience/salience.go` - `AccessSaturation` |
